...
if [[ -z "${MAX_MODEL_LEN}" ]]; then
  if [[ "${ASSUME_YES}" == "true" ]]; then
    ...
    reply=""
    if [[ -e /dev/tty ]]; then
      read -r -p "$(echo -e "${YELLOW}?${RESET} Enter the vLLM maximum context length (tokens) [default: 16384]: ")" reply < /dev/tty || reply=""
    else
      read -r -p "$(echo -e "${YELLOW}?${RESET} Enter the vLLM maximum context length (tokens) [default: 16384]: ")" reply || reply=""
    fi
    MAX_MODEL_LEN="${reply:-16384}"
  fi
fi
